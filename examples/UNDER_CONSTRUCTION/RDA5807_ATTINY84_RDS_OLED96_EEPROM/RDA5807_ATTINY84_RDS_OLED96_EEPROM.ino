...
void processRdsInfo() {
  ...
  if ( (currentmillis - delayProgramType) > 5000) {
    rx.convertToChar(rx.getRdsProgramType(), aux, 3, 0, ' ', false);

    showRdsText(0, 1, aux);
    delayProgramType = currentmillis;
  }
  ...
  if (utcTime != NULL && strlen(utcTime) > 8 && (currentmillis - delayUtcTime) > 60000) {
    strcpy(aux,strUtcTime);
    strcat(aux, " - L ");
    strcat(aux,strLocalTime);
    showRdsText(0, 3, aux);
...
void loop() {
  ...
  if (rx.getRdsReady()) {
    if (rx.hasRdsInfo()) {
      ...
      if (utcTime) strcpy(strUtcTime, utcTime);
      localTime = rx.getRdsLocalTime();
      if (localTime) strcpy(strLocalTime, localTime);
      processRdsInfo();
    }
  }
